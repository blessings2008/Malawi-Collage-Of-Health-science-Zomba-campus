...
export default function StudentsPage() {
  ...
  useEffect(() => {
    ...
    api.get('/api/periods').then((p) => {
      setAllPeriods(p);
      setPeriods(p.filter((x) => !x.is_locked));
      const current = p.find((x) => x.status === 'Current');
      if (current) setFilters((f) => (f.periodId ? f : { ...f, periodId: current.id }));
    }).catch(() => {});
  }, []);
    ...
    <div className="space-y-6">
      ...
      <div className="card p-4 grid md:grid-cols-6 gap-3">
        ...
        >
          ...
          {allPeriods.map((p) => (
            <option key={p.id} value={p.id}>{p.name}{p.status === 'Current' ? ' (Current)' : ''}</option>
          ))}
        </select>
      </div>
      <p className="text-xs text-navy-400 -mt-3">
        Showing allocation status for: <span className="font-semibold text-navy-600">
          {allPeriods.find((p) => p.id === filters.periodId)?.name || 'the current attachment period'}
        </span>
      </p>
      ...
      {isSuperAdmin && (
        <ManualAllocateModal
          ...
          periods={periods}
          districts={districts}
          onSaved={(usedPeriodId) => {
            setAllocatingStudent(null);
            setFilters((f) => ({ ...f, periodId: usedPeriodId }));
          }}
        />
      )}
