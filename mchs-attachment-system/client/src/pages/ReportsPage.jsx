...
export default function ReportsPage() {
  ...
  const EXTENSIONS = { pdf: 'pdf', excel: 'xlsx', docx: 'docx' };
  ...
  const download = async (typeKey, format) => {
    ...
    try {
      ...
      triggerBlobDownload(blob, `${typeKey}_report.${EXTENSIONS[format]}`);
    } catch (err) {
        ...
        {REPORT_TYPES.map((r) => (
          <div key={r.key} className="card p-5">
            ...
            <div className="flex gap-2 flex-wrap">
              <button
                className="btn-secondary flex-1 !py-2 text-xs min-w-[90px]"
              ...
              <button
                className="btn-secondary flex-1 !py-2 text-xs min-w-[90px]"
                disabled={downloading === `${r.key}-docx`}
                onClick={() => download(r.key, 'docx')}
              >
                {downloading === `${r.key}-docx` ? 'Preparing…' : 'Export Word'}
              </button>
            </div>
          </div>
        ))}
