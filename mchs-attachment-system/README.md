...
2. In the SQL Editor, run the migrations **in order** — open each file,
   ...
   - `supabase/migrations/0003_year_semester_program.sql`
...
- **Super Administrator** — full access, including user management,
  unlocking finalized allocations, and manually allocating or
  reallocating any individual student outside of a full engine run.
- **Administrator** — manages students, cohorts, districts, periods,
  allocations, and reports, including deleting student records and
  archiving/unarchiving cohorts.
- **Lecturer** — read-only: view students/allocations, search, filter,
  export reports.

## Academic Structure

Students are tracked by **Year + Semester** (six values: Year 1–3, each
with Semester 1 and 2) rather than year alone, and by **Program** —
constrained to the college's two actual programs: *Nursing and
Midwifery* and *Certificate in Midwifery Technicians*. Both are defined
as Postgres enums in `supabase/migrations/0003_year_semester_program.sql`
and as shared constants in `client/src/lib/constants.js`.
