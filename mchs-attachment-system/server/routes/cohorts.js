...
router.get('/', async (req, res) => {
  ...
  const withCounts = cohorts.map((cohort) => {
    ...
    const yearBreakdown = {};
    YEAR_SEMESTER_VALUES.forEach((y) => {
      yearBreakdown[y] = cohortStudents.filter((s) => s.year_of_study === y).length;
    });
    return {
      ...cohort,
...
// POST /api/cohorts/:id/unarchive — restore an archived cohort (admin+)
router.post('/:id/unarchive', requireRole('admin', 'super_admin'), async (req, res) => {
  const { data, error } = await supabaseAdmin
    .from('cohorts')
  ...
  await logAction({
    user: req.user,
    action: `unarchived cohort "${data.name}"`,
    entityType: 'cohort',
    entityId: data.id,
  });
