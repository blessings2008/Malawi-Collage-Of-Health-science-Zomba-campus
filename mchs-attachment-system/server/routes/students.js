...
router.get('/', async (req, res) => {
  const { search, year, cohortId, gender, districtId, status } = req.query;
  ...
  // Attach current-period allocation status/district (needed for districtId/status filters)
...
router.post('/', requireRole('admin', 'super_admin'), async (req, res) => {
  ...
  const { data, error } = await supabaseAdmin
    ...
    .insert({
      ...
      program: program || 'Clinical Medicine',
      cohort_id: cohortId,
      phone,
      email,
...
router.get('/import/template', requireRole('admin', 'super_admin'), async (req, res) => {
  ...
  sheet.columns = [
    ...
    { header: 'Year (Year 1/Year 2/Year 3)', key: 'yearOfStudy', width: 24 },
...
router.post('/import', requireRole('admin', 'super_admin'), upload.single('file'), async (req, res) => {
  ...
  sheet.eachRow((row, rowNumber) => {
    ...
    if (!['Year 1', 'Year 2', 'Year 3'].includes(yearOfStudy)) {
      rowErrors.push(`Row ${rowNumber}: invalid year "${yearOfStudy}".`);
      return;
    }
